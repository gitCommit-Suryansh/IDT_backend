...
router.get('/my-participations', verifyFirebaseToken, participationController.getMyParticipations);
router.get('/user/:userId/entries', verifyFirebaseToken, entryController.getUserEntries);
...
// Contest Actions
router.post('/:contestID/register', verifyFirebaseToken, participationController.registerForContest);
router.post('/:contestID/upload-entry', verifyFirebaseToken, (req, res, next) => { req.folderName = 'IDT-MEDIA/contest-entries'; next(); }, (req, res, next) => {
    upload.fields([{ name: 'images', maxCount: 3 }, { name: 'video', maxCount: 1 }])(req, res, (err) => {
        if (err) {
...
router.post('/:contestID/vote', verifyFirebaseToken, voteController.voteForEntry);
router.get('/:contestID/referral/:participationID', participationController.getReferralLink);
...
router.post('/:contestId/bookmark', verifyFirebaseToken, bookmarkController.toggleBookmark);
router.get('/user/bookmarks', verifyFirebaseToken, bookmarkController.getBookmarkedContests);
